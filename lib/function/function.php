    ...
    function user_reg($username,$email,$gender,$pass1,$cpass){
        ...
        if($user_check_count > 0){
            ...
            if($pass1 != $cpass){
                ...
                $insert_data = "INSERT INTO user_tbl(username,email,pass_user,gen<?php 
    include("config.php");
    use FTP\Connection;<?php 
    include("config.php");
    use FTP\Connection;
    session_start();
...
?>der,roll,user_status,join_date)VALUES('$username','$email','$pass1','$gender','user','1',NOW())";
                $insert_data_result = mysqli_query($con, $insert_data);
                header("location:../views/login.php");
            }
        } 
